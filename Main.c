...
{ // Ess estrutura que desenha o fundo colorido
	ALLEGRO_VERTEX v[] = {
		{.x = 0, .y = 0, .z = 0, .color = al_map_rgb(colorfundoRGB[1], colorfundoRGB[3], colorfundoRGB[2])},			// c.s.d
		{.x = WIDHT, .y = 0, .z = 0, .color = al_map_rgb(colorfundoRGB[2], colorfundoRGB[0], colorfundoRGB[3])},		// c.s.e
		{.x = 0, .y = HEIGHT, .z = 0, .color = al_map_rgb(colorfundoRGB[3], colorfundoRGB[2], colorfundoRGB[0])},		// c.i.d
		{.x = WIDHT, .y = HEIGHT , .z = 0, .color = al_map_rgb(colorfundoRGB[0], colorfundoRGB[1], colorfundoRGB[1])},  // c.i.e
	};
	...
	colorfundoRGB[2]++;
	colorfundoRGB[3]++;
	if (colorfundoRGB[0] == 255)
	{
		colorfundoRGB[0] = 70;
	}
	if (colorfundoRGB[1] == 255)
	{
		colorfundoRGB[1] = 130;
	}
	if (colorfundoRGB[2] == 255)
	{
		colorfundoRGB[2] = 30;
	}
	if (colorfundoRGB[3] == 255)
	{
		colorfundoRGB[3] = 70;
	}
}
void grade()
{
	ALLEGRO_COLOR ColorGrade = al_map_rgba(26, 26, 26, 200);

	float w = BLOCK*2;
	float h = BLOCK*2;
	// Linhas Horizontais
	for (int i = 0; i < HEIGHT / BLOCK - 3; i++)
	{
		al_draw_line(BLOCK*2, w, WIDHT- BLOCK*2, w, al_map_rgba(colorfundoRGB[0], colorfundoRGB[2], colorfundoRGB[3], 0), 2.0);
		al_draw_line(BLOCK*2, w, WIDHT- BLOCK*2, w, ColorGrade, 2.0);
		w += BLOCK;
	}
	// Linhas Verticais
	for (int index = 0; index < WIDHT / BLOCK - 3; index++)
	{
		al_draw_line(h, BLOCK*2, h, HEIGHT - BLOCK*2, al_map_rgba(colorfundoRGB[0], colorfundoRGB[2], colorfundoRGB[3], 0), 2.0);
		al_draw_line(h, BLOCK*2, h, HEIGHT - BLOCK*2, ColorGrade, 2.0);
		h += BLOCK;
	}
}
void area()
{
	...
	al_draw_filled_rectangle(BLOCK * 2, BLOCK * 2, WIDHT - (BLOCK * 2), HEIGHT - (BLOCK * 2), al_map_rgb(26, 26, 26));
}
...
int main()
{
	...
	teste_iniciar(al_init_image_addon(), "image addon"); // Isso e crucial para lidar com sprites
	...
	while (menu)
	{
		...
		while (GameLoop) 
		{
			...
			if (redraw && al_is_event_queue_empty(fila))
			{
				...
				fundo();
				area();
				grade();		 // desenha a grade de linhas
